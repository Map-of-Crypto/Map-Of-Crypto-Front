...
function PurchaseCard({ purchase: { purchaseId, merchantAddress, buyerAddress, accepted, deadline, ethPrice, ethFunded, trackingNumber }, onStateUpdate }) {
  ...
  const acceptPurchase = useCallback(async () => {
    ...
    try {
      ...
      await message.success({
        content: (
          <span>
            {`Success! Purchase with id: ${purchaseId} is accepted`}
          </span>
        ),
        duration: 5,
        key,
  ...
  const getActions = useMemo(() => {
    ...
    if (merchantAddress.toLowerCase() === address.toLowerCase() && !accepted && !ethFunded.isZero()) {
      actions.push(<Button type="primary" onClick={acceptPurchase}>Accept Purchase</Button>)
    }
  ...
  return (
    ...
    >
      <div className={styles.spaceBetween}>
        <div><span className={styles.label_inline}>Purchase ID:</span>{purchaseId.toString()}</div>
        <div><span className={styles.label_inline}>Accepted:</span>{accepted ? <CheckSquareFilled style={{ fontSize: '18px', color: green.primary }} /> : <CloseSquareFilled style={{ fontSize: '18px', color: red.primary }} />}</div>
      </div>
      <div><span className={styles.label}>Buyer Address:</span>{buyerAddress}</div>
      <div><span className={styles.label}>Merchant Address:</span>{merchantAddress}</div>
      <div><span className={styles.label}>Price (in crypto):</span>{utils.formatEther(ethPrice.toNumber())}</div>
      <div><span className={styles.label}>Funded (in crypto):</span>{utils.formatEther(ethFunded.toNumber())}</div>
      <div><span className={styles.label}>TrackingNumber:</span>{trackingNumber.toString()}</div>
      <div><span className={styles.label}>Deadline:</span>{deadline.toNumber()}</div>
    </Card>
  );
}
